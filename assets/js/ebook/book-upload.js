...
MBJS.AuthorBook.prototype = {
  ...
  basicSetups : function () {
    ...
    $("#ebook_file_00").change(function() {
      $('#ebook_upload_form_00').ajaxSubmit(options);
      return false;
    });

    $('#ebook_upload_form_00').submit(function() {
    ...
    function beforeSubmit() {
      ...
      if (window.File && window.FileReader && window.FileList && window.Blob) {
        if( !$('#ebook_file_00').val()) {
          $("#output").html("Are you kidding me?");
          return false
        }
        var fsize = $('#ebook_file_00')[0].files[0].size; //get file size
        var ftype = $('#ebook_file_00')[0].files[0].type; // get file type
